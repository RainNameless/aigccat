...
这个项目有一条硬规矩：**没跑通就不写「支持」**。所以上面那些 ❌，在 README 里就老实写着「未接入 / 未验收」。
