...
pub struct Builtin {
    ...
    /// 第二段密钥的环境变量（只有混元需要：SecretId:SecretKey 两段）
    pub key_env2: Option<&'static str>,
    pub docs: &'static str,
...
pub const BUILTIN: &[Builtin] = &[
    ...
    Builtin {
        ...
        key_env2: Some("HUNYUAN_SECRET_KEY"),
...
/// 轮询协议（rodin/hi3d/hunyuan 三家共用；model_jobs 的通用收尾按它驱动）。
/// 有意做成普通枚举而不是 trait：避免给 async 方法引 trait_object 的复杂度，
/// 每家传输层只需实现 `poll_once(&self, task_ref) -> Result<PollOutcome, String>`。
pub enum PollOutcome {
    Running(u64),
    Failed { error: String, credits: Option<serde_json::Value> },
    Success { glb: Vec<u8>, preview: Option<Vec<u8>>, warning: Option<String>, credits: Option<serde_json::Value> },
}
