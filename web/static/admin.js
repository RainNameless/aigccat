...
async function api(url,method='GET',body){const r=await fetch(url,{method,cache:'no-store',headers:{'Content-Type':'application/json'},...(body?{body:JSON.stringify(body)}:{})});const raw=await r.text().catch(()=>'');let j={};if(raw){try{const parsed=JSON.parse(raw);if(parsed&&typeof parsed==='object')j=parsed;}catch{j={error:raw.trim()};}}if(r.status===401){location.replace('/login.html?next='+encodeURIComponent(location.pathname+location.hash));throw Error('登录已过期');}if(!r.ok)throw Error(j.error||`请求失败（HTTP ${r.status}）`);return j;}
...
async function render(reset=true){const turn=++serial;if(reset)message('');content.hidden=false;
 ...
 if(tab==='accounts'){const studio=await fetch('/api/studio/status',{signal:AbortSignal.timeout(2500)}).then(r=>r.json()).catch(()=>({ready:false}));
   const catalog=await api('/api/settings/catalog');
...
function route(){
 ...
 const allowed=currentUser.role==='admin'?['overview','users','accounts','security','appearance','audit']:['security','appearance'];
