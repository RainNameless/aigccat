...
let dialog=null;
/** 把「创作设置」按钮挂到当前 DOM 的 #creation-settings（面板每次重渲染后都要重挂） */
export function mountCreationSettings(){
 const button=document.getElementById('creation-settings');
 if(!button||button.dataset.bound)return;
 button.dataset.bound='1';
 button.onclick=async()=>{try{
 if(!dialog){dialog=document.createElement('dialog');dialog.className='creation-settings-dialog';document.body.append(dialog);}
 ...
 }catch(e){dialog.querySelector('#creation-feedback').textContent=e.message;}finally{submit.disabled=false;}};
 dialog.showModal();
 }catch(e){const host=document.getElementById('parameters')||document.getElementById('chat-notice');if(host)host.insertAdjacentHTML('afterbegin',`<p class="help-line">${String(e.message).slice(0,160)}</p>`);}};
}
mountCreationSettings();
