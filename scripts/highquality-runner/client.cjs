...
const {resolveProxy}=require('../studio-runner/proxy.cjs');
...
async function client(){
 // 代理按需决定：显式 STUDIO_PROXY 优先，未设置时只在本机确有代理监听才用（见 studio-runner/proxy.cjs）
 ...
 async function model(url){
  ...
  if(b.length>512*1024*1024||b.length<20||b.toString('utf8',0,4)!=='glTF'||b.readUInt32LE(4)!==2||b.readUInt32LE(8)!==b.length)throw new StudioError('下载结果不是完整的 GLB 2 模型');
  return b;
 }
...
function payload(input,images){
 ...
 const value={face_limit:input.face_limit||20000,quad:false,visibility:'shareable',model_version:input.model_id,generate_parts:false,smart_poly:false,texture:!!input.texture,delight:true,pbr:!!input.texture&&!!input.pbr,texture_alignment:'original_image',texture_quality:'standard'};
