...
import json,sys,pathlib,urllib.request,base64,hashlib,subprocess
...
ROOT=pathlib.Path(__file__).resolve().parents[2];DIR=ROOT/'outputs/highquality-10'
...
api('/api/workbench/state',{'project_folders':{'highquality10':'高精度 8K · 10'},'asset_projects':{local['asset']:'highquality10'}},'PUT')
base='/api/assets/'+local['asset']
def put(key,data):return api(base+'/file/'+key,{'bytes_b64':base64.b64encode(data).decode()},'PUT')
if stage=='draft':
 put('source/tags.json',json.dumps({'tags':['demo','highquality10']}).encode())
 ...
 subprocess.run([sys.executable,str(pathlib.Path(__file__).with_name('inspect-glb.py')),str(model)]+(['--require-8k'] if stage in ['texture','final'] else []),check=True)
