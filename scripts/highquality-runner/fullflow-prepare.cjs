...
(async()=>{
 ...
 const dir=path.join(ROOT,'outputs/highquality-10',id),file=path.join(dir,stage+'.glb');
 ...
 try{await fs.writeFile(path.join(dir,stage+'-preview.png'),await preview(bytes));}
 catch(error){
  console.error('WebGL preview unavailable; rendering the same GLB with Blender CPU');
  const {execFileSync}=require('node:child_process');
  execFileSync('/Applications/Blender.app/Contents/MacOS/Blender',['--background','--factory-startup','--python',path.join(__dirname,'render-blender.py'),'--',file],{stdio:'inherit',timeout:240000});
 }
