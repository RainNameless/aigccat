...
try{const g=await new GLTFLoader().loadAsync('/studio-preview.glb');const r=new T.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});r.setSize(512,512);r.toneMapping=T.ACESFilmicToneMapping;document.body.append(r.domElement);const s=new T.Scene();s.background=new T.Color('#25282c');s.add(g.scene);const pm=new T.PMREMGenerator(r),room=new RoomEnvironment();s.environment=pm.fromScene(room,.04).texture;const light=new T.DirectionalLight(0xffffff,2);light.position.set(2,4,3);s.add(light,new T.HemisphereLight(0xffffff,0x445566,1));g.scene.updateMatrixWorld(true);g.scene.traverse(o=>{if(o.isSkinnedMesh)o.skeleton.update()});const b=new T.Box3().setFromObject(g.scene,true),size=b.getSize(new T.Vector3()),center=b.getCenter(new T.Vector3());const span=Math.max(size.x,size.y,size.z);const c=new T.PerspectiveCamera(40,1,span*.001,span*100);c.position.copy(center).add(new T.Vector3(1,.1,.18).normalize().multiplyScalar(span*1.8));c.lookAt(center);r.render(s,c);window.rendered=true;}catch(e){window.renderError=e.message;}
</script>`}));
