...
test('all entrances require login; shared users, separate cookies, CSRF and machine access',async()=>{
 ...
 const gateway=await createGateway({dir,upstream:'http://127.0.0.1:'+upstream.address().port,proxyToken:'private-proxy',automationToken:'private-machine',novnc:'http://127.0.0.1:'+upstream.address().port,publicOrigin:'https://aigccat.invalid',bootstrap:{username:'admin',password:'Example-test-password-2026'}});
 ...
 try{
  ...
  assert.equal((await request('/',{host:'aigccat.invalid'})).status,403);
  const remote={'x-aigccat-auth-proxy':'private-proxy'};
  assert.equal((await request('/',{host:'aigccat.invalid',headers:remote})).status,302);
  ...
  // 登录画面（noVNC）走同一个端口：没登录不给看，登录后转发到内部服务并剥掉前缀
  assert.equal((await request('/studio/vnc/vnc.html')).status,302);
  const login=async(host,headers={})=>request('/api/auth/login',{host,method:'POST',headers:{origin:host==='aigccat.invalid'?'https://aigccat.invalid':'http://'+host,'Content-Type':'application/json',...headers},body:JSON.stringify({username:'admin',password:'Example-test-password-2026'})});
  ...
  const pub=await login('aigccat.invalid',remote);assert.equal(pub.status,200);assert.match(pub.headers['set-cookie'][0],/^__Host-aigccat_session=.*Secure/);assert.equal(pub.json().user.id,local.json().user.id);
