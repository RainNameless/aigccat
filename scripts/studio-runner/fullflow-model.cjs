...
async function main(){
 ...
 const dir=path.join(ROOT,'outputs/fullflow-30',id),file=path.join(dir,task+'.private.json');
 const read=s=>fs.readFile(path.join(dir,s+'.private.json'),'utf8').then(JSON.parse);
 const item=JSON.parse(await fs.readFile(path.join(ROOT,'outputs/fullflow-30/manifest.json'))).cases.find(x=>x.id===id);
 ...
 try{
  if(!state.submission){
   ...
   if(stage==='geometry'){
    const symmetry=await c.call('/v2/studio/operation/symmetry_check',{image:{bucket:images[0].bucket,key:images[0].key}});
    route='/v2/studio/operation/multiview_to_model';body={face_limit:5000,quad:true,visibility:'shareable',model_version:'Nexus-v2.0-20260801',symmetry:symmetry.symmetry,image:images};
   }else if(stage==='texture'){
    route='/v2/studio/operation/texture_model';body={project_id:project,images,delight:true,part_names:JSON.parse(await fs.readFile(path.join(dir,'geometry-meshes.json'))),texture_alignment:'original_image',texture_quality:'standard'};
   }else if(stage==='rig'){
