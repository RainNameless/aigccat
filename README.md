...
![自定义模型服务](docs/screenshots/custom-model.png)

**模型分配**：生图（四视图）、文字 LLM（AI 绑骨）、识图（多模态）、3D 建模四条用途
各自指定一条模型，跨多家供应商随意挑选，改完立即生效。

**对话测试**：页底直接选一个模型试文字或图片，验证账号通不通。

![对话测试](docs/screenshots/chat-test.png)
...
建模不再区分「网页订阅 / API」两套界面 —— 选哪家就是哪家；Tripo 用订阅还是 API
由「当前账号」决定，卡片上直接标出来。每个参数旁有 ? 说明按钮（面数 / 几何精度 /
四边面 / 纹理 / PBR / 贴图质量 / 导出尺寸），右下角还有面向新手的向导。

![供应商卡片](docs/screenshots/provider-picker.png)

![参数帮助](docs/screenshots/parameter-help.png)

### 回收站永久删除

回收站里除了恢复，还可以**连 MinIO 数据一起真删**（双重确认，不可恢复）。

![回收站永久删除](docs/screenshots/trash-purge.png)
